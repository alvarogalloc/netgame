...
AssetManager::AssetManager(const std::string &assets_path)
{
...
std::size_t AssetManager::load(AssetType type, std::string path)
{
    ...
    case AssetType::Music: {
        m_tracks.push_back({++m_last_asset_id, {}});
        if (m_tracks.back().asset.openFromFile(filepath))
        {
            throw std::runtime_error{"Error loading Track: " + filepath};
        }
    }
    case AssetType::Texture: {
        m_textures.push_back({++m_last_asset_id, {}});
        if (m_textures.back().asset.loadFromFile(filepath))
        {
